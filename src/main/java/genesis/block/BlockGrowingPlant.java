...
public class BlockGrowingPlant extends BlockCrops implements IGrowable
{
	...
	/**
	 * Creates the BlockState for this BlockGrowingPlant. This was used to make the BlockState's properties depend on values given
	 * to the BlockGrowingPlant in the constructor.
	 *
	...
	/**
	 * Sets the position in a plant column that the "top" property should be true at. -1 = Default, the top property will be the top
	 * plant block in the column.
	 *
	...
	/**
	 * Sets a property to cause the entire plant column to break at once and drop only the first block's drops. false = Default,
	 * only the plant blocks above the broken one will break as well. true = All the plant blocks in the column will break at once
	 * and drop only the first block's drops.
	 *
	 * @return Returns this block.
	 */
	...
	/**
	 * Returns whether the block at pos should have the top property set to true. This takes this.topBlockPos into account as well,
	 * so it will not always be the highest block in the plant.
	 *
	 * @param worldIn
	 * @param pos
	...
	/**
	 * Gets the chance that a random tick of a crop block will result in the crop aging. 1 is added to the output to determine the
	 * actual chance.
	 */
	...
	/**
	 * Handles removal of a plant block. If breakTogether is true, it removes all plant blocks of this type in a column that they
	 * should grow in, optionally dropping items. Otherwise, it will handle setting the BlockState to acceptable values for this
	 * type of plant.
	 */
	 ...
	 * @param state
	 * @param chance
	 *            With a value less than 0, the block will be dropped. Otherwise, it will destroy the plant as it should normally,
	 *            but only if a player is not breaking it.
	 * @param fortune
	 */
	@Override
